...
// A timeline reads left-to-right / top-to-bottom oldest-first, the opposite
// of the store's newest-first order (which suits a feed, not a timeline).
const orderedUpdates = computed(() => [...progressStore.updates].reverse())
...
const lightboxUpdate = ref<ProgressUpdate | null>(null)
...
// Solid fills for the timeline dots — same status→color mapping as the
// badges above, just using the DEFAULT (solid) shade instead of the tint.
function dotColor(status: string) {
  if (status === 'verified') return 'bg-success'
  if (status === 'sent_back') return 'bg-error'
  return 'bg-pending'
}
...
<template>
  <div class="w-full px-[15%] py-8">
    ...
    <p
      v-else-if="!orderedUpdates.length"
    ...
    -->
    <div v-else class="relative flex flex-col gap-6 sm:flex-row sm:gap-4 sm:overflow-x-auto sm:pb-4">
      <div
      ...
      <div
        v-for="update in orderedUpdates"
        :key="update.id"
      ...
      >
        <span
          ...
          :class="dotColor(update.status)"
        />

        <div class="min-w-0 flex-1 sm:mt-2 sm:w-full">
          <p class="mb-2 text-[11px] text-muted sm:text-center">{{ formatDate(update.createdAt) }}</p>

          <div class="overflow-hidden rounded-lg border border-cream bg-white">
            <button
              type="button"
              class="block aspect-[4/3] w-full overflow-hidden bg-cream"
              @click="lightboxUpdate = update"
            >
              <img :src="update.images[0]?.thumb" alt="" class="h-full w-full object-cover" />
            </button>
    ...
    <PortfolioLightbox
      v-if="lightboxUpdate"
      :images="lightboxUpdate.images"
      :start-index="0"
      @close="lightboxUpdate = null"
    />
  </div>
</template>